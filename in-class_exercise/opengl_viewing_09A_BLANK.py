...
def main():
    ...
    # gluLookAt(0, 50, 0, 0, 0, 0, 0, 0, 1)                                   # Static view: set camera's eye, look-at, and view-up in the world
    initmodelMatrix = glGetFloat(GL_MODELVIEW_MATRIX)
    offset_z = 0
    gaze = np.array([-60, 10, -60])
    ...
    while True:
        ...
        glPushMatrix()
        glLoadMatrixf(initmodelMatrix)
        # gluLookAt(50 - zoom_dist, 0, 0, 0, 0, 0, 0, 1, 0) 
        scaled_gaze = gaze * zoom_dist
        gluLookAt(50+scaled_gaze[0], 0+scaled_gaze[1], 50+scaled_gaze[2], -10, 0, -10, 0, 1, 0)
        draw_Scarecrow()
        drawAxes()
        glPopMatrix()

...
def collisionTest_spheres(center1, radius1, center2, radius2):
    # calculate the squared distance between two centers
    # distance_squared = (center1 - center2) ** 2
    dx = center1[0] - center2[0]
    dy = center1[1] - center2[1]
    dz = center1[2] - center2[2]
    distance_squared = dx*dx + dy*dy + dz*dz
    # calculate the sum of two radii
    radii_sum_squared = (radius1 + radius2) ** 2

    # compared squared distance and squared radius sum
    # NOTE: It's more efficient compared to calculate the distance through square root
    collision_detected = False
    if distance_squared <= radii_sum_squared:
        collision_detected = True


    return collision_detected # MODIFY this based on the actual decision: True: Collision/ False: Not
...
def collisionTest_AABBs(min_coords1, max_coords1, min_coords2, max_coords2):
    ...
    # if object 1 is completely on the left of or on the right of object 2, no overlap
    if max_coords1[0] < min_coords2[0] or min_coords1[0] > max_coords2[0]:
        collision_detected = False
    # if object 1 is completely below or above object 2 
    elif max_coords1[1] < min_coords2[1] or min_coords1[1] > max_coords2[1]:
        collision_detected = False 
    # if object 1 is completely in behind or in front of object 2 
    elif max_coords1[2] < min_coords2[2] or  min_coords1[2] > max_coords2[2]:
        collision_detected = False

    # if max_coords1[i] < min_coords2[i] or min_coords1 > max_coords2:
    #     collision_detected = False
...
def collisionTest_sphereAABB(center1, radius1, min_coord2, max_coords2):
    ...
    # calculate the squared distance between closest point to the center


    # compare squared distance with squared radius  
    # NOTE: Again, it's more efficient compared to calculate the distance through square root

    return False # MODIFY this based on the actual decision: True: Collision/ False: Not
...
def main():
    ...
    BV2_type = "sphere" # bounding volume type: "sphere", "AABB"    
